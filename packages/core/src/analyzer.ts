...
export class CodeAnalyzer {
  ...
  private async discoverFiles(projectPath: string): Promise<CodeFile[]> {
  ...
  private generateSuggestions(file: CodeFile): Suggestion[] {
  ...
  private calculateScore(issues: Issue[], metrics: FileMetrics): number {
    let score = 100;
  ...
  private generateProjectSummary(results: AnalysisResult[]): ProjectSummary {
    const totalFiles = results.length;
    const totalIssues = results.reduce((sum, r) => sum + r.issues.length, 0);
    const criticalIssues = results.reduce((sum, r) => 
      sum + r.issues.filter(i => i.severity === 'critical').length, 0);
    const highPriorityIssues = results.reduce((sum, r) => 
  ...
  private initializeDefaultRules(): Rule[] {
    return [
      {
        id: 'console-log',
